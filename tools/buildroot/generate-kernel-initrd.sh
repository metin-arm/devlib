...
BUILDROOT_DIR="$(dirname "$0")/buildroot-v${BUILDROOT_VERSION}"
...
{
	mkdir -p "${BUILDROOT_DIR}/board/arm-power/${ARCH}/"
	cp -f "configs/post-build.sh" "${BUILDROOT_DIR}/board/arm-power/"
	cp -f "configs/${ARCH}/arm-power_${ARCH}_defconfig" "${BUILDROOT_DIR}/configs/"
	cp -f "configs/${ARCH}/linux.config" "${BUILDROOT_DIR}/board/arm-power/${ARCH}/"

	make -C "${BUILDROOT_DIR}" "arm-power_${ARCH}_defconfig"
}
